...
def define_fitness_calculator(protocols, etype):
...
def create(etype, cvode_active=True, dt=0.025, cvode_minstep=None):
    """Setup"""

    neuron_cell = model.template.create(etype)

    fitness_protocols = define_protocols(etype)
    fitness_calculator = define_fitness_calculator(fitness_protocols, etype)

    param_names = [param.name
                   for param in neuron_cell.params.values()
                   if not param.frozen]
